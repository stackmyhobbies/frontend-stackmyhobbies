...
const selectedItems = computed<T[]>({
  get: () => model.value,
  set: async (val) => {
    const unique = Array.from(new Map(val.map((item) => [item[valueKey], item])).values())
...
const removeItem = (item: T) => {
  const newVal = model.value.filter((p) => p[valueKey] !== item[valueKey])
  model.value = newVal
        ...
        >
          ...
          >
            <AppBadge
              ...
              :key="item[valueKey]"
              :color="getBadgeColor(item, index, valueKey.toString())"
            ...
            >
              ...
              >
                 ...
                 >
                  ...
                  <span
                    v-if="selected"
                    :class="resolvedSelectedTextClass"
                  >
                    <svg
