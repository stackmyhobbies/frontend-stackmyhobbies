  ...
  <div class="drawer-content">
    <div class="grid grid-cols-1 grid-rows-[auto_1fr_auto] min-h-screen">
      <nav class="navbar w-full bg-base-300 flex flex-row items-center justify-between p-3">
        ...
        <router-link v-if="!is_content_item_create" :to="{ name: 'content-item-create' }" class="btn btn-soft btn-info">
          <add-icon class="size-[1.5em]" />
          Add Hobby
        </router-link>

        <router-link v-else-if="!is_content_item_list" :to="{ name: 'content-item-list' }"
...
Add icon
      ...
      <dialog id="my_modal_5" class="modal modal-bottom sm:modal-middle" :open="toggleStore.isOpen">
        <div class="modal-box">
          <h3 class="text-lg font-bold">Hello!</h3>
          <p class="py-4">Press ESC key or click the button below to close</p>
          <div class="modal-action">
            <button class="btn" @click="toggleStore.close()">Close</button>
          </div>
        </div>
        <div class="modal-backdrop" @click="toggleStore.close()" />
      </dialog>
      <footer-component />
    </div>
  </div>
