<template>
  <div class="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
    <header-form :url="resetPasswordImage" alt="icon_reset_password" text_title="Reset your password" />

    <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
      <form class="space-y-6" @submit.prevent="onResetPassword">
        <div>
          <label for="password" class="block text-sm/6 font-medium text-base-content">New Password</label>
          <div class="mt-2">
            <input id="password" v-model="resetForm.password" ref="passwordInputRef" type="password" name="password"
              :class="[
        ...
        <div>
          <label for="confirmPassword" class="block text-sm/6 font-medium text-base-content">Confirm Password</label>
          <div class="mt-2">
            <input id="confirmPassword" v-model="resetForm.confirmPassword" type="password" name="confirmPassword"
              :class="[
                'block w-full rounded-md bg-white/5 px-3 py-1.5 text-base text-base-content outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 sm:text-sm/6',
...
watch(() => resetForm.confirmPassword, () => {
